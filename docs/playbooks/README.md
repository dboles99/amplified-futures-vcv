...
|---|---|---|
| [The Wall](the-wall.md) | StreetGridClock → Pulse → HarmonicPressure → StringMassCore × 2 → WallConductor → CollapseSat + FeedbackGovernor loop | Full massed-voice orchestra. Density sweeps and self-clocked collapse events. |
...
| [Percussion Slab](percussion-slab.md) | StreetGridClock → Pulse → Choke, DroneCore × 2, StringMassCore, Drift | Rhythmic structure driving drone muting. Clocked slab with Drift for texture dissolution. |
