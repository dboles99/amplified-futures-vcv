...
#
#   sh tests/golden_drone.sh            check against the reference
#   sh tests/golden_drone.sh capture    regenerate the reference
#
...
if [ "$1" = "capture" ]; then
	g++ $FLAGS -DAF_CAPTURE $INC -o tests/gd.exe tests/golden_drone.cpp $OBJS $LINK
	PATH="$RACK_APP:$PATH" ./tests/gd.exe > tests/golden_drone_data.hpp
	echo "captured tests/golden_drone_data.hpp"
else
	g++ $FLAGS $INC -o tests/gd.exe tests/golden_drone.cpp $OBJS $LINK
	PATH="$RACK_APP:$PATH" ./tests/gd.exe
fi
